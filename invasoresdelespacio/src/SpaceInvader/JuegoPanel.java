...
public class JuegoPanel extends JPanel implements ActionListener, KeyListener {
    ...
    // Cooldown para disparos del jugador
    private long tiempoUltimoDisparoJugador = 0;
    private static final int COOLDOWN_DISPARO_JUGADOR = 200; // milisegundos
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (spacePressed && !nave.isDisparando() && (ahora - tiempoUltimoDisparoJugador > COOLDOWN_DISPARO_JUGADOR)) {
            int disparoX = nave.getX() + nave.getAncho() * 2; // Centro considerando el escalado x4
            int disparoY = nave.getY();
            proyectilesJugador.add(new Proyectil(disparoX, disparoY, 0, (int)(-6*velocidadBase), Color.YELLOW, 6, 24));
            nave.setDisparando(true);
            tiempoUltimoDisparoJugador = ahora;
        }
        ...
        if (jefe != null) {
            jefe.mover();
            long ahoraJefe = System.currentTimeMillis();
            if (ahoraJefe - tiempoUltimaHabilidadJefe > 5000) {
                if (balasDisparadasEnRafaga < 3 && ahoraJefe - tiempoUltimoDisparoTriple > 1000) {
                    disparoTripleJefe(jefe);
                    balasDisparadasEnRafaga++;
                    tiempoUltimoDisparoTriple = ahoraJefe;
                }
                if (balasDisparadasEnRafaga >= 3) {
                    tiempoUltimaHabilidadJefe = ahoraJefe;
