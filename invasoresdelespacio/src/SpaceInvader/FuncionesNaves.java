...
public class FuncionesNaves {
    ...
    public static List<Object> generarEnemigos(int nivel, double velocidadBase) {
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            if (nivel % 10 == 0 && i == 0) {
                ...
                enemigos.add(new Jefe(250, 50, velocidadBase * 1.5));
            } else {
                String ruta;
                switch (tipo) {
                    case 1: ruta = "images/zombie.png"; break; // Normal
    ...
    public static List<Object> generarEnemigos(int nivel, double velocidadBase, int screenWidth, int screenHeight) {
        ...
        for (int i = 0; i < cantidad; i++) {
            ...
            if (nivel % 10 == 0 && i == 0) {
                enemigos.add(new Jefe(screenWidth/2-70, 50, velocidadBase * 1.5));
            } else {
                String ruta;
                switch (tipo) {
        ...
        for (int fila = 0; fila < formacion.length; fila++) {
            for (int col = 0; col < formacion[0].length; col++) {
                ...
                if (enemigo != null && enemigo.getTipo() == 3) {
                    ...
                    Rectangle rnav = new Rectangle(nave.getX(), nave.getY(), nave.getAncho() * 4, nave.getAlto() * 4);
                    ...
                    if (rpj.intersects(enemigo.getHitbox())) {
                        if (enemigo instanceof Jefe) {
                            ...
                            if (jefe.getVidas() <= 0) {
                                ...
                                levels.sumarPuntos(1000);
                                hud.sumarPuntos(1000);
                            }
