...
public class JuegoPanel extends JPanel implements ActionListener, KeyListener {
        ...
        for (int i = 0; i < filas; i++) {
            for (int j = 0; j < columnas; j++) {
                ...
                if (tipo > 0) {
                    ...
                    if (tipo == 4) formacion[i][j] = new Jefe(startX + j * sepX, startY + i * sepY, velocidadBase * 1.5, 50); // Jefe especial
                }
            }
        }
        return formacion;
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (jefe != null) {
            int barraAncho = 400;
            int barraAlto = 30;
            int barraX = getWidth()/2 - barraAncho/2;
            int barraY = 30;
            int vidasActuales = jefe.getVidas();
            int vidasMax = 50;
            int vidaAncho = (int)((double)vidasActuales / vidasMax * barraAncho);
            ...
            g.drawString("JEFE", barraX + barraAncho/2 - 30, barraY + barraAlto - 8);
        }
    }
